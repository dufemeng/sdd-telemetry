...
describe('ProfileProjectionRepository knowledge timeline', () => {
  it('groups knowledge recalls by relative path segment instead of materialized axis', async () => {
    const repository = createRepository(async (sql) => {
      ...
      return [
        ...
        },
        {
          ...
          knowledge_locator: '/wiki/domain-cashier/business/innerFlow/transfer-h5/pages/转入到活期.md',
        },
      ];
    });

    await expect(repository.getKnowledgeTimeline('sdd-default', 1, {
      rangeSinceDate: null,
      granularity: 'day',
