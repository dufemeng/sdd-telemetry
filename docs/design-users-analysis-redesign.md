...
- **新增独立个人画像页 `/sdd/users/:id`（懒加载，基线 A）**：两栏（左=他的需求，右=方法论完整度 + 活动时间线），节点回溯 prompt 全文（复用交互明细详情）。**行点击从「开抽屉」改为「跳画像页」**，对齐产出分析交互。
...
### 3.2 个人画像页 `/sdd/users/:id`（基线 A，与 `/work-items/:id` 同构两栏）
...
│ 采用爬坡: proposal D0 → design D3 → task D5 → code D12（12 天通链）│
├──────────────┬────────────────────────────────────────────────┤
│ 他的需求       │  方法论完整度 + 活动时间线                       │
│ ▸ cashier/..  │  完整度: proposal ✓ design ✓ task ✓ review —     │
│ ▸ unfreeze..◄ │  ● 05-28 14:22  Write design.md  bk:design wiki×3│
│ ▸ ...         │     "帮我设计冻结组件的…"  [展开全文]            │
│   (点→产出分析)│  ● 05-27 09:30  skill proposal   wiki×1         │
└──────────────┴────────────────────────────────────────────────┘
出链：需求行 → /work-items/:id ；技能 → 技能分析 ；wiki → 知识库分析
```

- **左栏**：这个人的需求列表（他参与/产出的 work items），点击出链 `/sdd/work-items/:id`（不在本页展开需求详情）。
- **右栏**：选中需求（或默认「全部活动」）的**活动时间线**——合并该用户的技能调用 / artifact 写入 / wiki 召回，按时间排；节点展开 → 抽屉复用交互明细详情渲染 **prompt + response 全文**（不重写）。顶部一条**方法论完整度 / 爬坡**带。
...
web/src/pages/sdd/users/
  ...
  UserProfilePage.tsx        新增：两栏（he的需求 + 活动时间线 + 成熟度带）
  ...
  components/
    UserWorkItemList.tsx     左栏需求列表（行出链产出分析）
    UserActivityTimeline.tsx 右栏活动时间线（节点 → 复用交互明细详情抽屉）
...
- 画像页**左栏需求** → 出链 `/sdd/work-items/:id`（人 → 需求方向）。
