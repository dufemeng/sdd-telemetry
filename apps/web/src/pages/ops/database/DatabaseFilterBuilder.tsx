...
export function DatabaseFilterBuilder({ columns, filters, onFiltersChange }: Props) {
  ...
  const startEdit = (group: FilterGroup) => {
    ...
    setDraft({
      ...group,
      conditions: group.conditions.map((c) => ({ ...c, id: c.id })),
    });
  };
  ...
  const confirm = () => {
    ...
    if (editingId) {
      onFiltersChange(
        compacted
          ? filters.map((g) => (g.id === editingId ? compacted : g))
          : filters.filter((g) => g.id !== editingId),
      );
    } else if (compacted) {
      onFiltersChange([...filters, compacted]);
    }
    close();
  };
  ...
  const remove = () => {
    if (editingId) {
      onFiltersChange(filters.filter((g) => g.id !== editingId));
    }
    close();
  };
        ...
        {filters.map((group, index) => (
          <div key={group.id} className="flex items-center gap-2">
            ...
            <button
              ...
              className="min-h-8 px-3 rounded-[4px] text-[12px] cursor-pointer text-[var(--color-primary)]"
              style={{ border: '1px solid rgba(250,255,105,0.30)', background: 'rgba(250,255,105,0.08)' }}
            >
