  ...
  if (group === 'registry') {
    return {
      ...
      rows: [
        ...
        configRow('getServing(profileId)', 'server/worker 按当前 profileId 取线上服务配置快照。', `当前返回 ${data.profile.profileId}`),
        configRow('listServing()', 'Profile 下拉框和配置列表读取 serving 配置集合。', 'DB 优先，builtin fallback 补齐'),
  ...
  if (group === 'runtime') {
    return {
      ...
      rows: [
        ...
        configRow('profile_source_matches', 'source-backed profile 的来源匹配物化结果，按配置版本隔离重匹配。', `${formatInteger(data.projection.matchCounts.sourceMatches)} 条匹配`),
