  ...
  return (
    ...
    >
        ...
        {group.conditions.map((condition) => {
          ...
          return (
            <div
              ...
              style={{ gridTemplateColumns: '220px 180px minmax(0,1fr) 28px' }}
            >
              <ColumnPicker
                columns={columns}
                value={condition.column}
                onChange={(name) => {
                  const next = columns.find((c) => c.columnName === name);
      ...
      <button
        ...
        className="flex items-center gap-1.5 self-start min-h-8 px-3 rounded-[4px] text-[12px] cursor-pointer text-[var(--color-secondary)] hover:text-[var(--color-primary)] hover:bg-[#202016]"
      ...
      <div className="flex items-center gap-2 pt-2" style={{ borderTop: '1px solid var(--color-border)' }}>
        ...
        <button
          type="button"
          onClick={onConfirm}
          className="min-h-8 px-4 rounded-[4px] text-[12px] font-bold cursor-pointer text-[var(--color-base)] bg-[var(--color-primary)] border-0"
        >
          确定
        </button>
      </div>
