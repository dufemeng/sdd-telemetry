...
export function PathDimensionDetailHeader({
  sourceNamespace,
  pathSegment,
}: {
  ...
  const overviewQuery = useKnowledgeOverview();
  const filters = { sourceNamespace, pathSegment };
  const reqQuery = useWikiRecallWorkItemRanking('all', filters);
  const timelineQuery = useWikiRecallTimeline('30d', 'day', filters);

  const pathDimension = useMemo<ProfileKnowledgePathDimensionSummary | undefined>(
    () =>
      overviewQuery.data?.pathDimensions.find(
        (dimension) =>
          dimension.sourceNamespace === sourceNamespace && dimension.pathSegment === pathSegment,
      ),
    [overviewQuery.data, sourceNamespace, pathSegment],
  );

  const reqItems = reqQuery.data?.items.slice(0, 5) ?? [];
  const d = pathDimension;
    ...
    <section className="grid gap-3 rounded-[6px] p-[14px]" style={CARD_STYLE}>
      <div className="flex flex-wrap items-center gap-x-6 gap-y-1 text-[12px] text-[var(--color-secondary)]">
        <KpiItem
          ...
          label="访问文档"
          value={formatInteger(d?.accessedDocs ?? 0)}
        />
        <KpiItem
          icon={<BookOpen size={14} />}
          label="知识访问"
          value={formatInteger(d?.accessCount ?? 0)}
        />
        <KpiItem
          icon={<Users size={14} />}
          label="读者"
          value={formatInteger(d?.distinctUsers ?? 0)}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_240px]">
        <div>
          <div className="mb-2 flex items-center gap-2 text-[12px] font-semibold text-[#f5f5f5]">
            ...
            <span className="text-[11px] font-normal text-[var(--color-muted)]">
              · {reqQuery.data?.total ?? reqItems.length} 个
            </span>
          </div>
          {reqQuery.isLoading ? (
            <div className="text-[12px] text-[var(--color-muted)]">加载中…</div>
          ) : reqItems.length === 0 ? (
            <div className="text-[12px] text-[var(--color-muted)]">暂无需求访问过该路径维度</div>
          ) : (
        ...
        <div>
          <div className="mb-2 text-[12px] font-semibold text-[#f5f5f5]">路径维度趋势(近30天)</div>
          <Sparkline points={timelineQuery.data?.buckets ?? []} />
        </div>
      </div>
    </section>
  );
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span
        ...
        style={{ color: color ?? '#f5f5f5', fontFamily: 'var(--font-mono)' }}
      >
        {value}
      </span>
    </div>
...
function Sparkline({ points }: { points: Array<{ t: string; accessCount: number }> }) {
  const totals = useMemo(() => {
    const map = new Map<string, number>();
    for (const p of points) {
      map.set(p.t, (map.get(p.t) ?? 0) + p.accessCount);
    }
  ...
  return (
    <div className="flex items-end gap-[2px]" style={{ height: 48 }}>
      ...
      <span
        className="ml-2 flex-none text-[11px] text-[var(--color-muted)]"
        style={{ fontFamily: 'var(--font-mono)' }}
      >
