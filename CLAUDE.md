...
- 离线 Docker 部署入口以 `README.md` 为准：本地可用 `pnpm docker:publish` 一键打包并上传 Release（底层仍为 `docker:package` + `docker:release`），无 Docker 的中转机用 `pnpm docker:relay` 下载 Release 并 scp，服务器用 `deploy/deploy-docker.sh`
